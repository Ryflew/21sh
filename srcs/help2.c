...
void	help_echo(void)
{
	...
	ft_putendl("\t\\c : Suppress the <newline> that otherwise follows the final argument in the output.");
	ft_putendl("\t\tAll characters following the \'\\c\' in the arguments shall be ignored.");
	...
	ft_putendl("\t\\0num : Write an 8-bit value that is the zero, one, two, or three-digit octal number num.");
	ft_putendl("\t$? : Write the last process return value.");
	ft_putendl("\t$[env variable name] : Write the selected env variable value.\n");
	ft_putstr(C_RESET);
}
...
{
	first_step("env");
	ft_putendl("\t- env [-i] [-u name] [name=value]... [utility [argument...]]\n");
